...
public class IntervalTest {
    ...
    /**
     * Test that when constructing Intervals, the start and end points are
     * picked out appropriately. When these values are manually altered, verify
     * the changes are properly reflected.
     */
    @Test
    public void testIntervalCreation() {
        Interval i = new Interval(1, 2, 3, 4, 5, 6, 7);
        // Constructor should pick out the smallest and largest values to build
        // the
        // interval
        testEnds(i, 1, 7);

        // Again, pick out the smallest and largest values when values are not
        // well
        // ordered
     ...
     * {@code ignoreOverlap} flag is set to true, so this is an exclusive
     * contains test.
     */
    @Test
    public void testIntervalContains() {
        final Interval i = new Interval(-5, 25);
    ...
    /**
     * Basic testing of the {@link Interval#intersects(Interval)} method. The
     * {@code ignoreOverlap} flag is set to true, so this is an exclusive
     * intersection test.
     */
    @Test
    public void testIntervalIntersects() {
        final Interval base = new Interval(0, 100);
    ...
    public void testEquality() {
        final Interval i1 = new Interval(-5, 5);
        final Interval i2 = new Interval(-5, 5);
        final Interval i3 = new Interval(100, 100);
     ...
     */
    private void testEnds(final Interval i, final int start, final int end) {
        assertTrue(i.min() == start);
        assertTrue(i.max() == end);
    }

    /**
     * Helper method to test {@link Interval#intersects(Interval)}. Tests both
     * directions of intersection between the intervals, to avoid bias.
     */
    private void testIntersect(final Interval base, final Interval test,
