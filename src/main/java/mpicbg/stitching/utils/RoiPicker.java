...
MouseListener, MouseMotionListener, MouseWheelListener, ToolWithOptions {
    ...
    public void mouseReleased(final MouseEvent e) {
        final ImageCanvas source = (ImageCanvas) e.getSource();
        if (source != canvas) {
            // We changed image window. Update fields accordingly
            final ImageWindow window = (ImageWindow) source.getParent();
        ...
        final int[] position =
                imp.convertIndexToPosition(imp.getCurrentSlice());

        final Set<Integer> matchedSet = new HashSet<Integer>();
        ...
        if (same(roiSet, matchedSet)) {
            ...
            // otherwise, update the cached indices and display the union of the
            // rois
            roiSet = matchedSet;
    ...
    // for macros
    public static void select(final String style, final String x1,
            final String y1, final String x2, final String y2,
            final String width, final String headLength) {
    }
     ...
     */
    private boolean same(final Set<?> set1, final Set<?> set2) {
    ...
    /**
     * Returns true iff the specified Roi contains the desired x and y
     * positions, and is located at the given position (C, Z, T).
     */
    private boolean containsPoint(final Roi r, final int[] position,
            final double x, final double y) {
        return r.getCPosition() == position[0]
                && r.getZPosition() == position[1]
     ...
     */
    private void selectRois(final RoiManager rm) {
        if (roiIndices.size() == 1) {
            ...
            IJ.showStatus(roiIndices.size()
                    + " overlapping Rois. Click to cycle...");
        }

        // Updates the displayed list of Rois to ensure the currently selected
        // Roi
        // is visible.
        try {
            final Field listField = RoiManager.class.getDeclaredField("list");
            listField.setAccessible(true);
            final int scrollIndex =
                    roiIndex == roiIndices.size() ? 0 : roiIndex;
    ...
    /**
     * Bumps the {@link #roiIndex}, or resets to 0 if == {@link #roiIndices}
     * size + 1. The extra index indicates multi-roi selection.
     */
