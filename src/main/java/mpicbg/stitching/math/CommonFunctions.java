...
public class CommonFunctions {
    ...
    public static ImagePlus loadImage(final String directory,
        ...
        return loadImage(directory, file, seriesNumber, "rgb");
    }

    public static ImagePlus loadImage(final String directory,
            final String file, final int seriesNumber, final String rgb) {
        ImagePlus imp = null;

        final String smallFile = file.toLowerCase();

        if (smallFile.endsWith("tif") || smallFile.endsWith("tiff")
                || smallFile.endsWith("jpg") || smallFile.endsWith("png")
                || smallFile.endsWith("bmp") || smallFile.endsWith("gif")
                || smallFile.endsWith("jpeg")) {
            imp = new Opener().openImage((new File(directory, file)).getPath());
        } else {
            imp = openLOCIImagePlus(directory, file, seriesNumber, rgb);
            if (imp == null) {
                imp =
                        new Opener().openImage((new File(directory, file))
                                .getPath());
            }
        }
    ...
    public static final void addHyperLinkListener(final MultiLineLabel text,
            final String myURL) {
    ...
    public static ImagePlus openLOCIImagePlus(final String path,
            final String fileName, final int seriesNumber, final String rgb) {
        return openLOCIImagePlus(path, fileName, seriesNumber, rgb, -1, -1);
    }

    public static ImagePlus openLOCIImagePlus(final String path,
            final String fileName, final int seriesNumber) {
        return openLOCIImagePlus(path, fileName, seriesNumber, "rgb", -1, -1);
    }

    public static ImagePlus openLOCIImagePlus(String path,
            final String fileName, final int seriesNumber, String rgb,
        ...
        try {
            ...
            // if loaded from a multiple series file (like LSM 710) select the
            // correct series
            if (seriesNumber >= 0) {
            ...
            if (!(pixelType == FormatTools.UINT8 || pixelType == FormatTools.UINT16)) {
                Log.error("PixelType " + pixelTypeString
                        + " not supported yet, returning. ");
                return null;
            }
             ...
             * Log.debug("depth: " + depth); Log.debug("timepoints: " +
             * timepoints); Log.debug("channels: " + channels);
             * Log.debug("images: " + num); Log.debug("image format: " +
             * formatType); Log.debug("bytes per pixel: " + bytesPerPixel);
            ...
            for (int z = start; z < end; z++) {
                final byte[][] b =
                        new byte[channels][width * height * bytesPerPixel];
                ...
                if (channels == 1) {
                    if (pixelType == FormatTools.UINT8) {
                        final ByteProcessor bp =
                                new ByteProcessor(width, height, b[0], null);
                        ...
                        final ShortProcessor sp =
                                new ShortProcessor(width, height, data, null);
    ...
    public static FloatArray3D zeroPad(final FloatArray3D ip, final int width,
        ...
        if (offsetX < 0) {
            IJ.error("Stitching_3D.ZeroPad(): Zero-Padding size in X smaller than image! "
                    + width + " < " + ip.width);
            return null;
        }

        if (offsetY < 0) {
            IJ.error("Stitching_3D.ZeroPad(): Zero-Padding size in Y smaller than image! "
                    + height + " < " + ip.height);
            return null;
        }

        if (offsetZ < 0) {
            IJ.error("Stitching_3D.ZeroPad(): Zero-Padding size in Z smaller than image! "
                    + depth + " < " + ip.depth);
            return null;
        }
        ...
        for (int z = 0; z < ip.depth; z++) {
            for (int y = 0; y < ip.height; y++) {
                for (int x = 0; x < ip.width; x++) {
                    image.set(ip.get(x, y, z), x + offsetX, y + offsetY, z
                            + offsetZ);
                }
            }
        }
    ...
    public static FloatArray3D[] zeroPadImages(final FloatArray3D img1,
            final FloatArray3D img2) {
    ...
    public static FloatArray2D zeroPad(final FloatArray2D ip, final int width,
            final int height) {
        final FloatArray2D image = new FloatArray2D(width, height);
    ...
    public static FloatArray2D[] zeroPadImages(FloatArray2D img1,
            FloatArray2D img2) {
        final int width = Math.max(img1.width, img2.width);
        final int height = Math.max(img1.height, img2.height);
        ...
        final FloatArray2D[] result = new FloatArray2D[2];
    ...
    public static void quicksort(final Quicksortable[] data, final int left,
            final int right) {
        ...
        final double x = data[(left + right) / 2].getQuicksortValue();
        ...
        do {
            ...
            if (i <= j) {
                final Quicksortable temp = data[i];
                data[i] = data[j];
                data[j] = temp;
                i++;
                j--;
    ...
    public static FloatArray2D pffft2D(final FloatArray2D values,
            final boolean scale) {
        ...
        final FftReal fft = new FftReal(width);
        ...
        final FftComplex fftc = new FftComplex(height);
    ...
    public static FloatArray2D pffftInv2D(final FloatArray2D values,
            final int nfft) {
        final int height = values.height;
        final int width = nfft;
        final int complexWidth = (width / 2 + 1) * 2;

        final FloatArray2D result = new FloatArray2D(width, height);
    ...
    public static FloatArray3D pffft3DMT(final FloatArray3D values,
            final boolean scale) {
        final int height = values.height;
        final int width = values.width;
        ...
        final FloatArray3D result =
                new FloatArray3D(complexWidth, height, depth);
        ...
        for (int ithread = 0; ithread < threads.length; ++ithread) {
            threads[ithread] = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    final float[] tempIn = new float[width];
                    float[] tempOut;
                    final FftReal fft = new FftReal(width);
        ...
        for (int ithread = 0; ithread < threads.length; ++ithread) {
            threads[ithread] = new Thread(new Runnable() {
                @Override
                public void run() {
                    final float[] tempIn = new float[height * 2];
                    float[] tempOut;
                    final FftComplex fftc = new FftComplex(height);
                        ...
                        if (z % numThreads == myNumber) {
                            for (int x = 0; x < complexWidth / 2; x++) {
                                ...
                                for (int y = 0; y < height; y++) {
                                    ...
                                    result.set(tempOut[y * 2 + 1], x * 2 + 1,
                                            y, z);
                                }
                            }
                        }
        ...
        for (int ithread = 0; ithread < threads.length; ++ithread) {
            threads[ithread] = new Thread(new Runnable() {
                @Override
                public void run() {
                    final float[] tempIn = new float[depth * 2];
                    float[] tempOut;
                    final FftComplex fftc = new FftComplex(depth);
                        ...
                        if (y % numThreads == myNumber) {
                            for (int x = 0; x < complexWidth / 2; x++) {
                                ...
                                for (int z = 0; z < depth; z++) {
                                    ...
                                    result.set(tempOut[z * 2 + 1], x * 2 + 1,
                                            y, z);
                                }
                            }
                        }
    ...
    public static FloatArray2D computePhaseCorrelationMatrix(FloatArray2D fft1,
            FloatArray2D fft2, final int width) {
        //
        // Do Phase Correlation
        //

        FloatArray2D pcm =
                new FloatArray2D(computePhaseCorrelationMatrix(fft1.data,
                        fft2.data, false), fft1.width, fft1.height);

        fft1.data = fft2.data = null;
        fft1 = fft2 = null;

        final FloatArray2D ipcm = pffftInv2D(pcm, width);
    ...
    public static FloatArray2D computeFFT(final FloatArray2D img) {
        final FloatArray2D fft = pffft2D(img, false);
        // img.data = null; img = null;

        return fft;
    }

    public static FloatArray3D computePhaseCorrelationMatrix(FloatArray3D fft1,
            FloatArray3D fft2, final int width) {
        //
        // Do Phase Correlation
        //

        FloatArray3D pcm =
                new FloatArray3D(computePhaseCorrelationMatrix(fft1.data,
                        fft2.data, false), fft1.width, fft1.height, fft1.depth);

        fft1.data = fft2.data = null;
        fft1 = fft2 = null;

        final FloatArray3D ipcm = pffftInv3DMT(pcm, width);
    ...
    public static FloatArray3D computeFFT(final FloatArray3D img) {
        final FloatArray3D fft = pffft3DMT(img, false);
        // img.data = null; img = null;

        return fft;
    }

    public static float[] computePhaseCorrelationMatrix(final float[] fft1,
            final float[] fft2, final boolean inPlace) {
        normalizeComplexVectorsToUnitVectors(fft1);
        normalizeComplexVectorsToUnitVectors(fft2);

        float[] fftTemp1 = fft1;
        final float[] fftTemp2 = fft2;
    ...
    public static void normalizeComplexVectorsToUnitVectors(
            final float[] complex) {
        final int wComplex = complex.length / 2;

        double length;

        for (int pos = 0; pos < wComplex; pos++) {
            length =
                    Math.sqrt(Math.pow(complex[pos * 2], 2)
    ...
    public static void complexConjugate(final float[] complex) {
        final int wComplex = complex.length / 2;
    ...
    public static float multiplyComplexReal(final float a, final float b,
            final float c, final float d) {
        return a * c - b * d;
    }

    public static float multiplyComplexImg(final float a, final float b,
            final float c, final float d) {
        return a * d + b * c;
    }

    public static float[] multiply(final float[] complexA,
            final float[] complexB, final boolean overwriteA) {
    ...
    public static FloatArray3D pffftInv3DMT(final FloatArray3D values,
            final int nfft) {
        final int depth = values.depth;
        final int height = values.height;
        final int width = nfft;
                        ...
                        if (y % numThreads == myNumber) {
                            for (int x = 0; x < complexWidth / 2; x++) {
                                ...
                                for (int z = 0; z < depth; z++) {
                                    ...
                                    values.set(tempOut[z * 2 + 1], x * 2 + 1,
                                            y, z);
                                }
                            }
                        }
                        ...
                        if (z % numThreads == myNumber) {
                            for (int x = 0; x < complexWidth / 2; x++) {
                                ...
                                for (int y = 0; y < height; y++) {
                                    ...
                                    values.set(tempOut[y * 2 + 1], x * 2 + 1,
                                            y, z);
                                }
                            }
                        }
        ...
        for (int ithread = 0; ithread < threads.length; ++ithread) {
            threads[ithread] = new Thread(new Runnable() {
                @Override
                public void run() {
                    final float[] tempIn = new float[complexWidth];
                    float[] tempOut;
                    final FftReal fft = new FftReal(width);
    ...
    final public static int[] getPixelMinRGB(final Object[] imageStack,
            final int width, final int height, final int depth, final int x,
            final int y, final int z, final double min) {
        ...
        final int[] pixelTmp = (int[]) imageStack[z];
    ...
    final public static float getPixelMin(final int imageType,
            final Object[] imageStack, final int width, final int height,
            final int depth, final int x, final int y, final int z,
            final double min) {
        if (x < 0 || y < 0 || z < 0 || x >= width || y >= height || z >= depth) {
            return (float) min;
        }
        ...
        if (imageType == ImagePlus.GRAY8) {
            final byte[] pixelTmp = (byte[]) imageStack[z];
    ...
    public static final double[] getPlanePosition(final IFormatReader r,
            final MetadataRetrieve retrieve, final int series, final int t) {
        return getPlanePosition(r, retrieve, series, t, false, false, false);
    }

    public static final double[] getPlanePosition(final IFormatReader r,
            final MetadataRetrieve retrieve, final int series, final int t,
            final boolean invertX, final boolean invertY,
            final boolean ignoreZStage) {
        // generate a mapping from native indices to Plane element indices
        final HashMap<Integer, Integer> planeMap =
    ...
    private static double getPosition(final Length planePos,
            final Length stageLabel, final boolean invert) {
        // check plane position
        if (planePos != null) {
            return invert ? -planePos.value().doubleValue() : planePos.value()
                    .doubleValue();
        }
