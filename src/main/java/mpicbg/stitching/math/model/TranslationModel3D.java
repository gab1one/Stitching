...
public class TranslationModel3D extends InvertibleModel {
    ...
    @Override
    final public int getMinSetSize() {
        return MIN_SET_SIZE;
    }

    // @Override
    @Override
    public float[] apply(final float[] point) {
        assert point.length == 3 : "3d translations can be applied to 3d points only.";

        return new float[] { point[0] + translation[0],
                point[1] + translation[1], point[2] + translation[2] };
    }
    ...
    public String toString() {
        return ("[1,3](" + translation[0] + "," + translation[1] + ","
                + translation[2] + ") " + cost);
    }
    ...
    final public void fit(final Collection<PointMatch> matches)
        ...
        if (matches.size() < MIN_SET_SIZE) {
            throw new NotEnoughDataPointsException(
                    matches.size()
                    + " data points are not enough to estimate a 3d translation model, at least "
                    + MIN_SET_SIZE + " data points required.");
        }
     ...
     */
    @Override
    final public void shake(final Collection<PointMatch> matches,
            final float scale, final float[] center) {
        // TODO If you ever need it, please implement it...
    }

    @Override
    public TranslationModel3D clone() {
        final TranslationModel3D tm = new TranslationModel3D();
