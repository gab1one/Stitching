...
/**
 * Abstract class for arbitrary transformation models to be applied to
 * {@link Point Points} in n-dimensional space.
 *
 * Model: R^n --> R^n
 *
...
public abstract class Model implements CoordinateTransform {
    ...
    /**
     * The cost depends on what kind of algorithm is running. It is always true
     * that a smaller cost is better than large cost
     */
    ...
    final public void setCost(final double c) {
        cost = c;
    }

    /**
     * @deprecated The term Error may be missleading---use {@link #getCost()}
     *             instead
     */
    @Deprecated
     ...
     *
     * @param m
     * @return false for error < 0, otherwise true if this.error is smaller than
     *         m.error
     */
    public boolean betterThan(final Model m) {
        if (cost < 0) {
            return false;
        }
        return cost < m.cost;
     ...
     *            point matches
     * @param scale
     *            gives a multiplicative factor to each dimensional distance
     *            (scales the amount of shaking)
     * @param center
     *            local pivot point for centered shakes (e.g. rotation)
     */
    abstract public void shake(Collection<PointMatch> matches, float scale,
            float[] center);
     ...
     * minimization. Use
     * {@link #ransac(Class, List, Collection, int, double, double) ransac} and/
     * or {@link #filter(Class, Collection, Collection)} to remove outliers from
     * your data points
     *
    ...
    final static public boolean test(final Model model,
            ...
            final Collection<PointMatch> inliers, final double epsilon,
            final double min_inlier_ratio) {
        inliers.clear();
     ...
     * @param candidates
     *            Candidate data points eventually inluding some outliers
     * @param inliers
     *            Remaining after the robust regression filter
     *
     */
    final static public <M extends Model> M filter(final Class<M> modelClass,
        ...
        do {
            ...
            // Log.debug( ( num_inliers - inliers.size() ) +
            // " candidates with e > " + t +
            // " removed by iterative robust regression." );
    ...
    /**
     * Estimate a {@link Model} from a set with many outliers by first filtering
     * the worst outliers with
     * {@link #ransac(Class, List, Collection, int, double, double) ransac}
     * \citet[{FischlerB81} tand filter potential outliers by robust iterative
     * regression.
     *
     * This method performs well on data sets with low amount of outliers. If
     * you have many outliers, you can filter those with a `tolerant' RANSAC
     ...
     *            Class of the model to be estimated
     * @param candidates
     *            Candidate data points inluding (many) outliers
     * @param inliers
     *            Remaining candidates after RANSAC
     * @param iterations
     * @param epsilon
     ...
     */
    final static public <M extends Model> M ransac(final Class<M> modelClass,
        ...
        while (i < iterations) {
            ...
            for (int j = 0; j < MIN_SET_SIZE; ++j) {
                ...
                do {
                    p =
                            candidates.get((int) (rnd.nextDouble() * candidates
                                    .size()));
            ...
            final ArrayList<PointMatch> temp_inliers =
                    new ArrayList<PointMatch>();
            m.fit(min_matches);
            int num_inliers = 0;
            boolean is_good =
                    test(m, candidates, temp_inliers, epsilon, min_inlier_ratio);
            while (is_good && num_inliers < temp_inliers.size()) {
                num_inliers = temp_inliers.size();
                m.fit(temp_inliers);
                is_good =
                        test(m, candidates, temp_inliers, epsilon,
                                min_inlier_ratio);
            }
            if (is_good && m.betterThan(model)
     ...
     * elaborate technique as follows: 1. Run RANSAC with large max_epsilon
     * resulting in many outliers included. 2. Get rid of the outliers using the
     * robust regression method of Dutter and Huber
     */
    static public <M extends Model> M filterRansac(final Class<M> modelType,
            final List<PointMatch> candidates,
            final Collection<PointMatch> inliers, final int iterations,
