...
public class ErrorStatistic {
    ...
    final public void add(final double new_value) {
        ...
        final double tmp = new_value - mean;
        ...
        if (sortedValues.size() % 2 == 0) {
            final int m = sortedValues.size() / 2;
    ...
    final public double getWideSlope(final int width)
            throws IndexOutOfBoundsException {
        if (width > slope.size()) {
            throw new IndexOutOfBoundsException(
                    "Cannot estimate a wide slope for width larger than than the number of sample.");
        }
        final ListIterator<Double> li = slope.listIterator(slope.size() - 1);
        int i = 0;
        double s = 0.0;
