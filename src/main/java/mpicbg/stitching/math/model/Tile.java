...
public class Tile {
    ...
    /**
     * The transformation {@link Model} of the {@link Tile}. All local points in
     * the {@link Tile} share (and thus determine) this common {@link Model}.
     */
    private Model model;
     ...
     * the corresponding local point in another {@link Tile}. {@link Tile}s are
     * perfectly registered if both {@link PointMatch.getP1 p1} and
    ...
    final public ArrayList<PointMatch> getMatches() {
        return matches;
    }

    final public void resetMatches() {
        matches.clear();
    }
    ...
    final public ArrayList<Tile> getConnectedTiles() {
        return connectedTiles;
    }

    final public void resetConnectedTiles() {
    ...
    /**
     * The transfer error of this {@link Tile}'s {@link Model} as estimated from
     * weighted square point correspondence displacement.
     */
     ...
     *
     * @param width
     *            width of the {@link Tile} in world unit dimension (e.g.
     *            pixels).
     * @param height
     *            height of the {@link Tile} in world unit dimension (e.g.
     *            pixels).
     * @param model
     *            the transformation {@link Model} of the {@link Tile}.
     */
    public Tile(final float width, final float height, final Model model) {
        this.width = width;
        this.height = height;
        this.model = model;
     ...
     *            the transformation {@link Model} of the {@link Tile}.
     */
    public Tile(final double width, final double height, final Model model) {
        this.width = (float) width;
        this.height = (float) height;
        this.model = model;

        lc =
                new float[] { this.width / 2.0f - 1.0f,
     ...
     *
     * @param more
     *            the {@link PointMatch}es to be added.
     ...
     *
     * @param match
     *            the {@link PointMatch} to be added.
     * @return True if the list changed as a result of the call.
     */
    ...
    /**
     * Apply the current {@link Model} to all local point coordinates. Update
     ...
     *
     */
    final public void updateByStrength(final float amount) {
        // tile center world coordinates
        wc = model.apply(lc);
     ...
     *
     * @param max_num_tries
     *            maximal number of tries before returning false (which means
     *            "no better model found")
     * @param scale
     *            strength of shaking
     * @return true if a better model was found
     */
    final public boolean diceBetterModel(final int max_num_tries,
            final float scale) {
        // store old model
        final Model old_model = model;
    ...
    /**
     * Update the transformation {@link Model}. That is, fit it to the current
     * set of {@link PointMatch}es.
     */
    final public void fitModel() throws NotEnoughDataPointsException {
        model.fit(matches);
        // update(); // Do not update! The user wants to decide about the update
        // strategy (weighted or unweighted)
    }
     ...
     */
    final private int traceConnectedGraph(final ArrayList<Tile> graph) {
        graph.add(this);
     ...
     * the connections between tiles during minimization. TODO Check if this is
     * a good idea... TODO What about the size of a detection, shouldn't it be
     * used as a weight factor as well?
     *
     * Change 2007-10-27 Do not normalize by changing the weight, correpondences
     * are weighted by feature scale.
     *
     * @param o
     * @param matches
     */
    final public void connect(final Tile o, final Collection<PointMatch> matches) {
    ...
    final static public ArrayList<ArrayList<Tile>> identifyConnectedGraphs(
        ...
        final ArrayList<ArrayList<Tile>> graphs =
                new ArrayList<ArrayList<Tile>>();
        int numInspectedTiles = 0;
        A: for (final Tile tile : tiles) {
            for (final ArrayList<Tile> knownGraph : graphs) {
                if (knownGraph.contains(tile)) {
                    continue A;
                }
            }
            final ArrayList<Tile> current_graph = new ArrayList<Tile>();
     ...
     */
    final public boolean isInside(final float[] point) throws Exception {
        final float[] local = ((InvertibleModel) model).applyInverse(point);
        return (local[0] >= 0.0f && local[0] < width && local[1] >= 0.0f && local[1] < height);
    }
     ...
     * | +-+ +-----+
     *
     * Test for edge intersections and points: If only one single edge of this
     * intersects with one of t both tiles intersect.
     *
     * @param t
     *            the other {@link Tile}
     */
    final public boolean intersects(final Tile t) throws Exception {
