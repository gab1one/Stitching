...
public class GridType {
    ...
    final public static String[] choose1 = new String[] { "Grid: row-by-row", // 0
    ...
    public GridType() {
        ...
        images[0][0] =
                GenericDialogPlus.createImageIcon(getClass().getResource(
                        "/images/row1.png"));
        images[0][1] =
                GenericDialogPlus.createImageIcon(getClass().getResource(
                        "/images/row2.png"));
        ...
        // John Lapage added this: use the correct image if sequential images is
        // selected
        ...
        final GenericDialogPlus gd =
                new GenericDialogPlus("Grid/Collection stitching");
        gd.addChoice("Type", choose1,
                choose1[Stitching_Grid.defaultGridChoice1]);

        if (!IJ.macroRunning()) {
            gd.addChoice(
                    "Order",
                    choose2[Stitching_Grid.defaultGridChoice1],
                    choose2[Stitching_Grid.defaultGridChoice1][Stitching_Grid.defaultGridChoice2]);

            try {
                final ImageIcon display =
                        new ImageIcon(
                                images[Stitching_Grid.defaultGridChoice1][Stitching_Grid.defaultGridChoice2]
                                        .getImage());
                final JLabel label = gd.addImage(display);

                // start the listener
                imageSwitch((Choice) gd.getChoices().get(0), (Choice) gd
                        .getChoices().get(1), images, display, label);
            } catch (final Exception e) {
                gd.addMessage("");
                gd.addMessage("Cannot load images to visualize the grid types ... ");
            }
        } else {
            // the interactive changing is not compatible with the macro
            // language,
            // thats why we show all possible options and figure out what was
            // meant
    ...
    static {
        // 0: Grid: row-by-row
        choose2[0] =
                new String[] { "Right & Down                ", "Left & Down",
                "Right & Up", "Left & Up" };
        // 1: Grid: column-by-column
        choose2[1] =
                new String[] { "Down & Right                ", "Down & Left",
                "Up & Right", "Up & Left" };
        ...
        choose2[6] =
                new String[] { "Defined by TileConfiguration",
    ...
    protected final void imageSwitch(final Choice choice1,
            final Choice choice2, final ImageIcon[][] images,
            final ImageIcon display, final JLabel label) {
