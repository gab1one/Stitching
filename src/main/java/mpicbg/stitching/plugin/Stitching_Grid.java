...
public class Stitching_Grid implements PlugIn {
    ...
    public void run(final String arg0) {
        ...
        final GenericDialogPlus gd =
                new GenericDialogPlus("Grid stitching: "
                        + GridType.choose1[gridType] + ", "
                        + GridType.choose2[gridType][gridOrder]);
        ...
        if (gridType == 6 && gridOrder == 1) {
            ...
            // Modified by John Lapage: copying the general setup for Unknown
            // Positions option
            ...
            if (gridType == 6) {
                ...
                gd.addStringField("Output_textfile_name",
                        defaultTileConfiguration, 50);
            }
        }

        gd.addChoice("Fusion_method", CommonFunctions.fusionMethodListGrid,
                CommonFunctions.fusionMethodListGrid[defaultFusionMethod]);
        gd.addNumericField("Regression_threshold", defaultRegressionThreshold,
                2);
        gd.addNumericField("Max/avg_displacement_threshold",
                defaultDisplacementThresholdRelative, 2);
        gd.addNumericField("Absolute_displacement_threshold",
                defaultDisplacementThresholdAbsolute, 2);
        // added by John Lapage: creates text box in which the user can set
        // which range to compare within. Would be nicer as an Integer.
        if (gridType == 7) {
        ...
        if (gridType < 5) {
            gd.addCheckbox(
                    "Compute_overlap (otherwise use approximate grid coordinates)",
            ...
            gd.addCheckbox(
                    "Compute_overlap (otherwise trust coordinates in the file)",
                    defaultComputeOverlap);
        ...
        gd.addCheckbox(
                "Use_virtual_input_images (Slow! Even slower when combined with subpixel accuracy during fusion!)",
                defaultVirtualInput);
        gd.addChoice("Computation_parameters", CommonFunctions.cpuMemSelect,
                CommonFunctions.cpuMemSelect[defaultMemorySpeedChoice]);
        gd.addChoice("Image_output", resultChoices,
                resultChoices[defaultResult]);
        gd.addMessage("");
        ...
        if (gridType < 5) {
            ...
            if (seperateOverlapY) {
                ...
                overlapX =
                        overlapY =
                        defaultOverlapY =
                        defaultOverlapX = gd.getNextNumber();
                overlapX /= 100.0;
                overlapY = overlapX;
            }
        ...
        params.relativeThreshold =
                defaultDisplacementThresholdRelative = gd.getNextNumber();
        params.absoluteThreshold =
                defaultDisplacementThresholdAbsolute = gd.getNextNumber();
        // Added by John Lapage: sends user specified range to the parameters
        // object
        if (gridType == 7) {
            params.seqRange =
                    (int) (defaultSeqRange = Math.round(gd.getNextNumber()));
        }

        final boolean addTilesAsRois =
                params.addTilesAsRois =
                defaultAddTilesAsRois = gd.getNextBoolean();
        // Modified by John Lapage (rearranged). Copies the setup for Unknown
        // Positions. User specifies this with all other options.
        if (gridType == 5 || gridType == 7) {
        ...
        params.cpuMemChoice =
                defaultMemorySpeedChoice = gd.getNextChoiceIndex();
        ...
        if (params.virtual) {
            ...
            if (params.subpixelAccuracy
                    && params.fusionMethod != CommonFunctions.fusionMethodListGrid.length - 1) {
        ...
        if (params.fusionMethod != CommonFunctions.fusionMethodListGrid.length - 1
            ...
            if (defaultOutputDirectory == null
                    || defaultOutputDirectory.length() == 0) {
                defaultOutputDirectory = defaultDirectory;
            }

            final GenericDialogPlus gd2 =
                    new GenericDialogPlus("Select output directory");
            gd2.addDirectoryField("Output_directory", defaultOutputDirectory,
                    60);
            gd2.showDialog();
            ...
            params.outputDirectory =
                    defaultOutputDirectory = gd2.getNextString();
        } else {
        ...
        // added by John Lapage: sets the parameters object to recognise that
        // Sequential File pairing should be performed
        if (gridType == 7) {
        ...
        if (gridType < 5) {
            elements =
                    getGridLayout(grid, gridSizeX, gridSizeY, overlapX,
                            overlapY, directory, filenames, startI, startX,
                            startY, params.virtual, ds);
        } else if (gridType == 5 || gridType == 7) {
            elements = getAllFilesInDirectory(directory, confirmFiles);
        } else if (gridType == 6 && gridOrder == 1) {
            // metadata
            elements =
                    getLayoutFromMultiSeriesFile(seriesFile, increaseOverlap,
                            ignoreCalibration, invertX, invertY, ignoreZStage,
                            ds);
        } else if (gridType == 6) {
        ...
        for (final ImageCollectionElement element : elements) {
            ...
            final int lastNumChannels = numChannels;
            final int lastNumTimePoints = numTimePoints;
            ...
            if (imp.getNSlices() > 1) {
                if (gridType >= 5) {
                    Log.info("" + imp.getWidth() + "x" + imp.getHeight() + "x"
                            + imp.getNSlices() + "px, channels=" + numChannels
                            + ", timepoints=" + numTimePoints + " (" + time
                            + " ms)");
                }
                is3d = true;
            } else {
                if (gridType >= 5) {
                    Log.info("" + imp.getWidth() + "x" + imp.getHeight()
                            + "px, channels=" + numChannels + ", timepoints="
                            + numTimePoints + " (" + time + " ms)");
                }
                is2d = true;
            }
        ...
        // the dimensionality of each image that will be correlated (might still
        // have more channels or timepoints)
        ...
        if (params.fusionMethod != CommonFunctions.fusionMethodListGrid.length - 1) {
            ...
            if (params.outputDirectory == null) {
                ...
                Log.info("Fuse & Write to disk (into directory '"
                        + new File(params.outputDirectory, "")
                        .getAbsolutePath() + "') ...");
            }
            ...
            if (overlapX == 0 && overlapY == 0
                    ...
                    && params.subpixelAccuracy == false && grid.getType() < 4) {
                final GenericDialogPlus gd3 =
                        new GenericDialogPlus("Use fast fusion algorithm");
            ...
            if (is32bit) {
                imp =
                        Fusion.fuse(new FloatType(), images, models,
                                params.dimensionality, params.subpixelAccuracy,
                                params.fusionMethod, params.outputDirectory,
                                noOverlap, false, params.displayFusion);
            } else if (is16bit) {
                imp =
                        Fusion.fuse(new UnsignedShortType(), images, models,
                                params.dimensionality, params.subpixelAccuracy,
                                params.fusionMethod, params.outputDirectory,
                                noOverlap, false, params.displayFusion);
            } else if (is8bit) {
                imp =
                        Fusion.fuse(new UnsignedByteType(), images, models,
                                params.dimensionality, params.subpixelAccuracy,
                                params.fusionMethod, params.outputDirectory,
                                noOverlap, false, params.displayFusion);
            } else {
            ...
            Log.info("Finished fusion (" + (System.currentTimeMillis() - time)
                    + " ms)");
            Log.info("Finished ... ("
            ...
            if (addTilesAsRois) {
                ...
                if (imp == null) {
                    // Save the rois
                    rm.runCommand("save", new File(params.outputDirectory,
                            "tile_rois.zip").getAbsolutePath());
                } else {
                    // Display our rois
                    rm.runCommand("Show All");
                    // Activate the RoiPicker tool
                    final Toolbar bar = Toolbar.getInstance();
                    final int roiPickerId =
                            bar.getToolId(new RoiPicker().getToolName());
                    if (roiPickerId >= 0) {
