...
import mpicbg.stitching.stitching.fusion.OverlayFusion;
...
public class Stitching_Pairwise implements PlugIn {
    ...
    public void run(final String arg0) {
         ...
         */
        final GenericDialog gd1 =
                new GenericDialog("Paiwise Stitching of Images");
        ...
        gd1.addChoice("Second_image (to register)", imgList,
                imgList[defaultImg2]);
        ...
        ImagePlus imp1 =
                WindowManager.getImage(idList[defaultImg1 =
                gd1.getNextChoiceIndex()]);
        ImagePlus imp2 =
                WindowManager.getImage(idList[defaultImg2 =
                gd1.getNextChoiceIndex()]);
        ...
        final String error = testRegistrationCompatibility(imp1, imp2);
        ...
        gd2.addChoice("Fusion_method", fusionMethodList,
                fusionMethodList[defaultFusionMethod]);
        gd2.addStringField("Fused_image name: ",
                imp1.getTitle() + "<->" + imp2.getTitle(), 20);
        gd2.addSlider("Check_peaks", 1, 100, defaultCheckPeaks);
        gd2.addCheckbox("Ignore zero values when fusing",
        ...
        gd2.addChoice("Registration_channel_image_1 ", channels1,
                channels1[defaultChannel1]);
        gd2.addChoice("Registration_channel_image_2 ", channels2,
                channels2[defaultChannel2]);

        if (imp1.getNFrames() > 1) {
            gd2.addChoice("Time-lapse_registration",
                    CommonFunctions.timeSelect,
                    CommonFunctions.timeSelect[defaultTimeSelect]);
        }
        ...
        if (simpleFusion) {
            params.fusionMethod =
                    ...
                    + (CommonFunctions.fusionMethodList.length - CommonFunctions.fusionMethodListSimple.length);
        } else {
            params.fusionMethod =
        ...
        params.checkPeaks =
                defaultCheckPeaks = (int) Math.round(gd2.getNextNumber());
        params.ignoreZeroValuesFusion =
                defaultIgnoreZeroValues = gd2.getNextBoolean();
        params.displayFusion =
                Stitching_Grid.defaultDisplayFusion = gd2.getNextBoolean();
        params.computeOverlap = defaultComputeOverlap = gd2.getNextBoolean();
        params.subpixelAccuracy =
                defaultSubpixelAccuracy = gd2.getNextBoolean();
        ...
        if (!params.computeOverlap && params.timeSelect > 0) {
            Log.warn("You chose to not compute overlap, ignoring the option '"
                    + CommonFunctions.timeSelect[params.timeSelect] + "'!");
            params.timeSelect = defaultTimeSelect = 0;
            Log.warn("Instead we will '"
                    + CommonFunctions.timeSelect[params.timeSelect] + "'");
        }

        if (params.timeSelect > 0) {
            final GenericDialog gd3 =
                    new GenericDialog("Details for timelapse stitching");

            gd3.addChoice("Computation parameters",
                    CommonFunctions.cpuMemSelect,
                    CommonFunctions.cpuMemSelect[defaultMemorySpeedChoice]);
            // gd3.addNumericField( "Regression_Threshold",
            // defaultRegressionThreshold, 2 );
            gd3.addNumericField("Max/Avg Displacement Threshold",
                    defaultDisplacementThresholdRelative, 2);
            gd3.addNumericField("Absolute Avg Displacement Threshold",
            ...
            params.cpuMemChoice =
                    defaultMemorySpeedChoice = gd3.getNextChoiceIndex();
            // params.regThreshold = defaultRegressionThreshold =
            // gd3.getNextNumber();
            params.relativeThreshold =
                    defaultDisplacementThresholdRelative = gd3.getNextNumber();
            params.absoluteThreshold =
                    defaultDisplacementThresholdAbsolute = gd3.getNextNumber();
        }
