...
public class Downsampler {
     ...
     *            Base image height
     */
    public void getInput(final int imgWidth, final int imgHeight) {
        originalWidth = imgWidth;
        originalHeight = imgHeight;
        final GenericDialogPlus gdDownSample =
                new GenericDialogPlus("Downsample");
        ...
        final Vector<?> fields = gdDownSample.getNumericFields();
        ...
        if (gdDownSample.wasOKed()) {
            ...
            final double width = gdDownSample.getNextNumber();
            final double height = gdDownSample.getNextNumber();
            final String method = gdDownSample.getNextChoice();
            final String average =
                    gdDownSample.getNextBoolean() ? " average" : "";
            params =
                    "width=" + width + " height=" + height + average
     ...
     */
    public void run(final ImagePlus... imps) {
        checkInit();
     ...
     */
    public void run(final ImageCollectionElement element) {
        checkInit();
     ...
     * {@link #getInput(int, int)} call.
     */
    public void run(final List<ImageCollectionElement> elements) {
        checkInit();
    ...
    private void checkInit() {
        if (!hasInput()) {
            throw new IllegalStateException("Downsample failed: please"
     ...
     */
    private class DownsampleFocusListener implements
    ...
    private class DownsampleTextListener implements TextListener {
        ...
        public void textValueChanged(final TextEvent e) {
            ...
            try {
                ...
                final Double xText = Double.parseDouble(xField.getText());
                final Double yText = Double.parseDouble(yField.getText());

                if (source == widthField && widthField == fieldWithFocus) {
                    final double newX = widthText / originalWidth;
                    xField.setText(String.valueOf(newX));
                } else if (source == heightField
                        && heightField == fieldWithFocus) {
                    final double newY = heightText / originalHeight;
                    yField.setText(String.valueOf(newY));
                } else if (source == xField && xField == fieldWithFocus) {
                    final int newWidth =
                            (int) Math.round(xText * originalWidth);
                    widthField.setText(String.valueOf(newWidth));
                } else if (source == yField && yField == fieldWithFocus) {
                    final int newHeight =
                            (int) Math.round(yText * originalHeight);
