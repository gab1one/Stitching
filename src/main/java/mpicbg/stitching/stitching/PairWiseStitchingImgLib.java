...
public class PairWiseStitchingImgLib {
    public static PairWiseStitchingResult stitchPairwise(final ImagePlus imp1,
            final ImagePlus imp2, Roi roi1, Roi roi2, final int timepoint1,
        ...
        if (canWrap) {
            if (imp1.getType() == ImagePlus.GRAY32) {
                final Image<FloatType> image1 =
                        getWrappedImageFloat(imp1, params.channel1, timepoint1);

                if (imp2.getType() == ImagePlus.GRAY32) {
                    result =
                            performStitching(
                                    image1,
                                    getWrappedImageFloat(imp2, params.channel2,
                                            timepoint2), params);
                ...
                final Image<UnsignedShortType> image1 =
                        getWrappedImageUnsignedShort(imp1, params.channel1,
                                timepoint1);
                ...
                final Image<UnsignedByteType> image1 =
                        getWrappedImageUnsignedByte(imp1, params.channel1,
                                timepoint1);
            ...
            final ImageFactory<UnsignedByteType> imgFactoryByte =
                    new ImageFactory<UnsignedByteType>(new UnsignedByteType(),
                            StitchingParameters.phaseCorrelationFactory);
            final ImageFactory<UnsignedShortType> imgFactoryShort =
                    new ImageFactory<UnsignedShortType>(
                            new UnsignedShortType(),
                            StitchingParameters.phaseCorrelationFactory);
            final ImageFactory<FloatType> imgFactoryFloat =
                    new ImageFactory<FloatType>(new FloatType(),
                            StitchingParameters.phaseCorrelationFactory);

            if (imp1.getType() == ImagePlus.GRAY32) {
                final Image<FloatType> image1 =
                        getImage(imp1, roi1, imgFactoryFloat, params.channel1,
                                timepoint1);
                ...
                final Image<UnsignedShortType> image1 =
                        getImage(imp1, roi1, imgFactoryShort, params.channel1,
                                timepoint1);
                ...
                final Image<UnsignedByteType> image1 =
                        getImage(imp1, roi1, imgFactoryByte, params.channel1,
                                timepoint1);
    ...
    public static <T extends RealType<T>, S extends RealType<S>> PairWiseStitchingResult performStitching(
        ...
        final PairWiseStitchingResult result =
                computePhaseCorrelation(img1, img2, params.checkPeaks,
                        params.subpixelAccuracy);

        return result;
    }

    public static <T extends RealType<T>, S extends RealType<S>> PairWiseStitchingResult computePhaseCorrelation(
            final Image<T> img1, final Image<S> img2, final int numPeaks,
     ...
     *            - the {@link ImagePlus}
     * @param imgFactory
     *            - the {@link ImageFactory} defining wher to put it into
     * @param channel
     *            - which channel (if channel=0 means average all channels)
     * @param timepoint
     *            - which timepoint
     *
     * @return - the {@link Image} or null if it was not an ImagePlus.GRAY8,
     *         ImagePlus.GRAY16 or ImagePlus.GRAY32
     */
    public static <T extends RealType<T>> Image<T> getImage(
            final ImagePlus imp, Roi roi, final ImageFactory<T> imgFactory,
    ...
    /**
     * Averages all channels into the target image. The size is given by the
     * dimensions of the target image, the offset (if applicable) is given by an
     * extra field
     *
     * @param target
     *            - the target Image
     * @param offset
     *            - the offset of the area (might be [0,0] or [0,0,0])
     * @param imp
     *            - the input ImagePlus
     * @param timepoint
     *            - for which timepoint
     ...
     */
    public static <T extends RealType<T>> boolean averageAllChannels(
            final Image<T> target, final int[] offset, final ImagePlus imp,
            final int timepoint) {
        final int numChannels = imp.getNChannels();

        if (imp.getType() == ImagePlus.GRAY8) {
            final ArrayList<Image<UnsignedByteType>> images =
                    new ArrayList<Image<UnsignedByteType>>();
            ...
            final ArrayList<Image<UnsignedShortType>> images =
                    new ArrayList<Image<UnsignedShortType>>();
            ...
            final ArrayList<Image<FloatType>> images =
     ...
     * @return true if successful, false if the ImagePlus type was unknow
     */
    public static <T extends RealType<T>> boolean fillInChannel(
            final Image<T> target, final int[] offset, final ImagePlus imp,
            final int channel, final int timepoint) {
        if (imp.getType() == ImagePlus.GRAY8) {
            final ArrayList<Image<UnsignedByteType>> images =
     ...
     */
    protected static <T extends RealType<T>, S extends RealType<S>> void averageAllChannels(
            final Image<T> target, final ArrayList<Image<S>> sources,
            final int[] offset) {
        ...
        for (int ithread = 0; ithread < threads.length; ++ithread) {
            threads[ithread] = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    final LocalizableCursor<T> targetCursor =
                            target.createLocalizableCursor();

                    // the input cursors
                    final ArrayList<LocalizableByDimCursor<S>> sourceCursors =
                            new ArrayList<LocalizableByDimCursor<S>>();
    ...
    /**
     * return an {@link Image} of {@link UnsignedByteType} as input for the
     ...
     *            - the {@link ImagePlus}
     *
     * @return - the {@link Image} or null if it was not an ImagePlus.GRAY8 or
     *         if channel = 0
     */
    public static Image<UnsignedByteType> getWrappedImageUnsignedByte(
            final ImagePlus imp, final int channel, final int timepoint) {
        if (channel == 0 || imp.getType() != ImagePlus.GRAY8) {
            return null;
        }
        return ImageJFunctions.wrapByte(Hyperstack_rearranger.getImageChunk(
                imp, channel, timepoint));
    }

    /**
     * return an {@link Image} of {@link UnsignedShortType} as input for the
     * PhaseCorrelation. If no rectangular roi is selected, it will only wrap
     * the existing ImagePlus!
     *
     * @param channel
     ...
     *            - the {@link ImagePlus}
     *
     * @return - the {@link Image} or null if it was not an ImagePlus.GRAY16 or
     *         if channel = 0
     */
    public static Image<UnsignedShortType> getWrappedImageUnsignedShort(
            final ImagePlus imp, final int channel, final int timepoint) {
        if (channel == 0 || imp.getType() != ImagePlus.GRAY16) {
            return null;
        }
        return ImageJFunctions.wrapShort(Hyperstack_rearranger.getImageChunk(
                imp, channel, timepoint));
    }

    /**
     * return an {@link Image} of {@link FloatType} as input for the
     * PhaseCorrelation. If no rectangular roi is selected, it will only wrap
     * the existing ImagePlus!
     *
     * @param channel
     ...
     *            - the {@link ImagePlus}
     *
     * @return - the {@link Image} or null if it was not an ImagePlus.GRAY32 or
     *         if channel = 0
     */
    public static Image<FloatType> getWrappedImageFloat(final ImagePlus imp,
            final int channel, final int timepoint) {
        if (channel == 0 || imp.getType() != ImagePlus.GRAY32) {
            return null;
        }
        return ImageJFunctions.wrapFloat(Hyperstack_rearranger.getImageChunk(
                imp, channel, timepoint));
    }
