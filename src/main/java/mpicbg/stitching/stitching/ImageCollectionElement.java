...
public class ImageCollectionElement {
    ...
    public void setImagePlus(final ImagePlus imp) {
        ...
        if (imp.getNSlices() == 1) {
            ...
            size =
                    new int[] { imp.getWidth(), imp.getHeight(),
                    imp.getNSlices() };
        }
    }

    public ImagePlus open(final boolean virtual) {
        if (imp != null && isVirtual() == virtual) {
            return imp;
        }
        ...
        try {
            ...
            if (virtual) {
                ...
                imp = BF.openImagePlus(file.getAbsolutePath()); // this worked,
                // so we keep it
                // (altough both
                // should be the
                // same)
            ...
            if (imp.length > 1) {
                Log.error("LOCI does not open the file '"
                        + file
                        + "'correctly, it opens the image and splits it - maybe you should convert all input files first to TIFF?");
            ...
            if (imp[0].getNSlices() == 1) {
                ...
                size =
                        new int[] { imp[0].getWidth(), imp[0].getHeight(),
                        imp[0].getNSlices() };
            }
