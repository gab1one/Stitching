...
public class OverlayFusion {
    protected static <T extends RealType<T> & NativeType<T>> CompositeImage createOverlay(
            final T targetType, final ImagePlus imp1, final ImagePlus imp2,
            final InvertibleBoundable finalModel1,
            final InvertibleBoundable finalModel2, final int dimensionality) {
    ...
    public static <T extends RealType<T> & NativeType<T>> ImagePlus createReRegisteredSeries(
            final T targetType, final ImagePlus imp,
            final ArrayList<InvertibleBoundable> models,
        ...
        // estimate the boundaries of the output image and the offset for fusion
        // (negative coordinates after transform have to be shifted to 0,0,0)
        ...
        for (int t = 1; t <= numImages; ++t) {
            for (int c = 1; c <= imp.getNChannels(); ++c) {
                ...
                final Img<FloatType> in =
                        ImageJFunctions.convertFloat(Hyperstack_rearranger
                                .getImageChunk(imp, c, t));

                fuseChannel(out, Views.interpolate(Views.extendZero(in),
                        new NLinearInterpolatorFactory<FloatType>()), offset,
                        models.get(t - 1));
                ...
                try {
                    ...
                    for (int z = 1; z <= out.dimension(2); ++z) {
                        stack.addSlice(imp.getTitle(), outImp.getStack()
                                .getProcessor(z));
                    }
    ...
    public static <T extends RealType<T> & NativeType<T>> CompositeImage createOverlay(
        ...
        for (int i = 0; i < images.size(); ++i) {
            ...
            for (int c = 1; c <= imp.getNChannels(); ++c) {
                ...
                final Img<FloatType> in =
                        ImageJFunctions.convertFloat(Hyperstack_rearranger
                                .getImageChunk(imp, c, timepoint));

                fuseChannel(out,
                        Views.interpolate(Views.extendZero(in), factory),
                        offset, models.get(i + (timepoint - 1) * numImages));
                try {
                    final ImagePlus outImp =
                            ((ImagePlusImg<?, ?>) out).getImagePlus();
                    for (int z = 1; z <= out.dimension(2); ++z) {
        ...
        // convertXYZCT ...
        ImagePlus result =
                new ImagePlus("overlay " + images.get(0).getTitle() + " ... "
                        + images.get(numImages - 1).getTitle(), stack);
     ...
     *            - the transformation
     */
    protected static <T extends RealType<T>> void fuseChannel(
            final Img<T> output, final RealRandomAccessible<FloatType> input,
            final double[] offset, final InvertibleCoordinateTransform transform) {
        ...
        for (int ithread = 0; ithread < threads.length; ++ithread) {
            threads[ithread] = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    final RealRandomAccess<FloatType> in =
                            input.realRandomAccess();
         ...
         *
         * try { while ( out.hasNext() ) { out.fwd();
         *
         * for ( int d = 0; d < dims; ++d ) tmp[ d ] = out.getPosition( d ) +
         * offset[ d ];
         *
         * transform.applyInverseInPlace( tmp );
         *
         * in.setPosition( tmp ); out.getType().setReal( in.getType().get() ); }
         * } catch (NoninvertibleModelException e) { Log.error(
         * "Cannot invert model, qutting." ); return; }
         */
    }

    /**
     * Rearranges an ImageJ XYCZT Hyperstack into XYZCT without wasting memory
     * for processing 3d images as a chunk, if it is already XYZCT it will
     * shuffle it back to XYCZT
     *
     * @param imp
     *            - the input {@link ImagePlus}
     * @return - an {@link ImagePlus} which can be the same instance if the
     *         image is XYZT, XYZ, XYT or XY - otherwise a new instance
    ...
    public static ImagePlus switchZCinXYCZT(final ImagePlus imp) {
            ...
            for (int c = 1; c <= numChannels; ++c) {
                for (int z = 1; z <= numZStacks; ++z) {
                    ...
                    final ImageProcessor ip =
                            imp.getStack().getProcessor(index);
        ...
        final CompositeImage composite =
                CompositeImageFixer.makeComposite(result);

        return composite;
    }

    /**
     * Rearranges an ImageJ XYCTZ Hyperstack into XYCZT without wasting memory
     * for processing 3d images as a chunk, if it is already XYCTZ it will
     * shuffle it back to XYCZT
     *
     * @param imp
     *            - the input {@link ImagePlus}
     * @return - an {@link ImagePlus} which can be the same instance if the
     *         image is XYC, XYZ, XYT or XY - otherwise a new instance
     *         containing the same processors but in the new order XYZCT
     */
