...
public class BlendingPixelFusion implements PixelFusion {
     ...
     *
     * @param images
     *            - all input images (the position in the list has to be the
     *            same as Id provided by addValue!)
     */
    public BlendingPixelFusion(
            final ArrayList<? extends ImageInterpolation<?>> images) {
     ...
     *            - which percentage of the image should be blended ( e.g. 0,3
     *            means 15% on the left and 15% on the right)
     */
    private BlendingPixelFusion(
            final ArrayList<? extends ImageInterpolation<?>> images,
                    final double fractionBlended) {
        this.images = images;
        percentScaling = fractionBlended;

        numDimensions = images.get(0).getImg().numDimensions();
        numImages = images.size();
        dimensions = new long[numImages][numDimensions];
        ...
        border = new double[numDimensions];
    ...
    public void addValue(final double value, final int imageId,
        ...
        final double weight =
                Math.max(
                        0.00001,
                        computeWeight(localPosition, dimensions[imageId],
    ...
    public PixelFusion copy() {
        return new BlendingPixelFusion(images);
    }
    ...
    final public static double computeWeight(final double[] location,
        ...
        for (int dim = 0; dim < location.length; ++dim) {
            ...
            double value =
                    Math.max(1, Math.min(localImgPos - border[dim] + 1,
                            (dimensions[dim] - 1) - localImgPos - border[dim]
                                    + 1));

            final float imgAreaBlend =
                    Math.round(percentScaling * 0.5f * dimensions[dim]);
